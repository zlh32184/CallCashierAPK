...
//	                	}
	...
	protected void downLoadApk(FileApp fileApp) {  
	    ...
	    pd.setMessage("正在下载"+fileApp.getAppName());  
	    pd.show();  
	    new Thread(){  
	        @Override  
	        public void run() {  
...
//	        			pd.setMax(conn.getContentLength());
	    	    		...
	    	    		try{
	    	                ...
	    	                HttpEntity httpEntity = httpResponse.getEntity();
	    	                if (httpEntity != null){
	    	                	Log.i("cheny", httpResponse.getHeaders("File-Size").length+"");
	    	                	if(httpResponse.getHeaders("File-Size")!=null && httpResponse.getHeaders("File-Size").length!=0){
	    	                		Log.i("cheny","aaa");
	    	                		int downloadCount = 0;  
	    		        		    ...
	    		        		    fileSize = new Integer(httpResponse.getHeaders("File-Size")[0].getValue());
	    		        		    pd.setMax(fileSize);
	    		        		    while ((bytesRead = is.read(buffer, 0, 1024)) != -1) {
	    		        			   os.write(buffer, 0, bytesRead);
	    		        				...
	    		        				if((downloadCount == 0)||(int) (downLoadFileSize*100/fileSize)-2>downloadCount){   
	    		                            downloadCount += 2;  
	    		        					sendMsg(DOWN_ING);// 更新进度条
	    		        				}
	    		        		    }
	    		        		    os.close();
	    		        		    is.close();
