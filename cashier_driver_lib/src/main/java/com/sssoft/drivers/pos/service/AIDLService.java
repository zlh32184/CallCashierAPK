...
{	
    ...
    //联迪硬件服务
    private static AidlDeviceService bocDeviceService;
	private static MyBocConnection myBocConnection = new MyBocConnection();
	...
	public static class MyBocConnection implements ServiceConnection
	{
		@Override
		public void onServiceConnected(ComponentName componentName, IBinder iBinder)
		{
			bocDeviceService = AidlDeviceService.Stub.asInterface(iBinder);
		}

		@Override
		public void onServiceDisconnected(ComponentName componentName)
		{
			bocDeviceService = null;
		}
	}
	public static void BocInit(){
		myBocConnection = new MyBocConnection();
		Intent intent = new Intent();
		intent.setAction("com.boc.DeviceService");
		intent.setPackage("com.boc.spos.service");
		mContext.bindService(intent, myBocConnection, Context.BIND_AUTO_CREATE);
	}
	public static void BocUnBind(){
		if(bocDeviceService!=null) {
			//mContext.unbindService(myBocConnection);
		}
	}
	public static AidlDeviceService getBocDeviceService() {
		return bocDeviceService;
	}
	public static void setBocDeviceService(AidlDeviceService deviceService) {
		AIDLService.bocDeviceService = deviceService;
	}
