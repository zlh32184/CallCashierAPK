...
import com.zacloud.deviceservice.aidl.UPCardListener;
import com.zacloud.deviceservice.aidl.PBOCHandler;
import com.zacloud.deviceservice.aidl.OnlineResultHandler;
import com.zacloud.deviceservice.aidl.CandidateAppInfo;
...
interface IPBOC {
     ...
     * <li>needCDCVM(boolean) - 是否支持小额免签免密</li>
    ...
    /**
     * 更新AID参数
     * @param operation - 操作标识，1-新增，2-删除，3-清空
	...
	/**
	 * 获取PBOC流程数据，如卡号、有效期、卡片序列号等。
	 * EMV必须执行到相应流程才能获取到对应的卡片数据，否则返回空。
	 * @param tagName - 标签名称
	 * <ul>
     * <li>PAN - 卡号</li>
     * <li>TRACK2 - 磁道二数据</li>
	...
	/**
     * 设置EMV内核日志打印/显示开关
     * @param debugFlag - 0x00表示关闭所有调试输出
     * bit8置位 - 输出EMV内核日志
     * bit1置位 - 输出EMV流程logcat日志
     */
    void setDebugShow(byte debugFlag);


    /*
        读取卡号，支持三种界面
    */
    void readCardNo(int timeOut, ReadCardListener listener);
